<template>
  <div class="setup">
    ...
    <div class='setPlayerInfo' v-for="(p, index) in players" :key="index">
      ...
      <b-form-input class='playerName' v-model="p.name" placeholder="Enter Player Name" type='text'></b-form-input>
      <label class=labelClass>Set Player Score</label>
      <b-form-input class='playerScore' v-model="p.score" placeholder="Update Score" type='number'></b-form-input>
      <label class=labelClass>Set Player Colour</label>
      <b-form-input class='playerScore' v-model="p.colour" type='color'></b-form-input>
    </div>
  </div>
</template>
...
export default {
  ...
  computed: {
    ...
    maxPlayers() {
      return this.$store.state.maxPlayers
    }
  }
}
